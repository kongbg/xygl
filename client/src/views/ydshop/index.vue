<template>
  <div class="ydshop-container">
    <el-card class="ydshop-card">
      <template #header>
          ...
          <template #default="{ row }">
            <template v-if="row.isExtract">
              <el-button 
                link 
                type="primary" 
                @click="$router.push(`/ydgood?shopId=${row.shopId}`)"
              >
                {{ row.productCount }}
              </el-button>
            </template>
...
const fetchShops = async () => {
  try {
    const { data } = await request.get('/ydshops', {
      params: {
        page: page.value,
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        if (isEdit.value) {
          await request.put(`/ydshops/${currentId.value}`, form.value)
        } else {
          await request.post('/ydshops', form.value)
        }
...
const handleToggleStatus = async (row) => {
  try {
    await request.put(`/ydshops/${row.shopId}`, { status: row.status ? 0 : 1 })
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    try {
      await request.delete(`/ydshops/${row.shopId}`)
...
const startParseGoodDetail = async (shopId) => {
  ...
  try {
    const { data } = await request.post(`/ydshops/${shopId}/parseGoodDetail`)
    clearInterval(timer)
...
const startParseGoods = async (shopId) => {
  ...
  try {
    const { data } = await request.post(`/ydshops/${shopId}/parseGood`)
    clearInterval(timer)
    parseProgress.value = 100
    parseStatus.value = 'success'
    parseMessage.value = '解析成功！'
...
const startParsing = async (shopId) => {
  ...
  try {
    const { data } = await request.post(`/ydshops/${shopId}/parse`)
    clearInterval(timer)
    parseProgress.value = 100
    parseStatus.value = 'success'
    parseMessage.value = '解析成功！'
